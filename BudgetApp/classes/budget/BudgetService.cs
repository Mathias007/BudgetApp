...
{  
    public abstract class BudgetService : IBudgetService
    {
        ...
        public static Dictionary<int, Transaction> LoadTransactionList(string fileName)
        {
            ...
            if (!File.Exists(UtilitiesLibrary.GetDatabasePath(fileName)))
            {
                using FileStream createStream = File.Create(UtilitiesLibrary.GetDatabasePath(fileName));
            }
            else
            {
                returnDictionary = JsonConvert.DeserializeObject<Dictionary<int, Transaction>>(File.ReadAllText(UtilitiesLibrary.GetDatabasePath(fileName)));
            }
        ...
        public static Dictionary<int, Category> LoadCategoryList(string fileName)
        {
            ...
            {
                returnDictionary =  JsonConvert.DeserializeObject<Dictionary<int, Category>>(File.ReadAllText(UtilitiesLibrary.GetDatabasePath(fileName)));
            }
            if (returnDictionary == null)
            {
        ...
        public static Dictionary<int, User> LoadUserList(string fileName)
        {
            ...
            {
                returnDictionary = JsonConvert.DeserializeObject<Dictionary<int, User>>(File.ReadAllText(UtilitiesLibrary.GetDatabasePath(fileName)));
            }
            if (returnDictionary == null)
            {
        ...
        {
            File.WriteAllText(UtilitiesLibrary.GetDatabasePath(fileName), JsonConvert.SerializeObject(transactionsList));
        }

        public static void SaveUserList(Dictionary<int, User> usersList, string fileName)
        {
            File.WriteAllText(UtilitiesLibrary.GetDatabasePath(fileName), JsonConvert.SerializeObject(usersList));
        }
        public static void SaveCategoryList(Dictionary<int, Category> categoryList, string fileName)
        {
            File.WriteAllText(UtilitiesLibrary.GetDatabasePath(fileName), JsonConvert.SerializeObject(categoryList));
        }
    }
}
